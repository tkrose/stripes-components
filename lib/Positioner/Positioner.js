...
class Positioner extends React.Component {
  ...
  updatePosition() {
    this.popRAF = requestAnimationFrame(() => {
      ...
      let finalStyle = Object.assign({}, newStyle, chk);

      if (this.props.renderLocal) {
        finalStyle.top =  finalStyle.top - this._SPBounds.top;
        finalStyle.left = finalStyle.left - this._SPBounds.left;
      }
      ...
      renderedOverlay = ReactDOM.createPortal(
        <div className={css.localOverlayContainer} ref={(t) => { this._localContainer = t; }}>
          <Transition key={this._containerKey} appear timeout={70} in={this.state.in} onExited={onHide}>
            { status => (
              <div className={this.getClassNames(status)} style={this.state.style}>
                {open &&
                  <div tabIndex="-1" className={css.PositionerOverlay} ref={(t) => { this._overlayContainer = t; }}>
                    {this.props.overlay}
                  </div>
                }
              </div>
