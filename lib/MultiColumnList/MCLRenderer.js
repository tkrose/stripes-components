...
class MCLRenderer extends React.Component {
  ...
  getContainerStyle() {
    ...
    /* avoid setting width to containing elements so that 
    text aligns with text of other content */
  ...
  getBodyStyle() {
    ...
    /* avoid setting width to containing elements so that 
    text aligns with text of other content */
    // bodyStyle.width = `${this.props.width + 20}px` || '100%';
  ...
  getContainerClass() {
    return classnames(
      css.mclContainer,
      {[css.smallGutter]:this.props.smallGutter},
    );
  }
